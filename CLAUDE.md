...
- Bump `CONTENT_UPDATED` in `src/data/site.ts` whenever page content or the
  calculation model changes. It is the sitemap `lastmod`, so it should move when
  the content moves and stay put otherwise.
